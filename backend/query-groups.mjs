...
for (const g of groups) {
  ...
  console.log(`"${g.name}" (${g.id}) → ${u ? (u.name||u.username)+' ['+u.id+']' : 'kein Mitglied'}`);
}
...
for (const g of groups) {
  ...
  console.log(`${g.name} (Code: ${g.code}) → Owner: ${owner ? (owner.name || owner.username) + ' [' + owner.id + ']' : 'kein Owner'}`);
}
await prisma.$disconnect();
