...
async function runTests() {
  ...
  expect(uploadResult.beforePickerDisabled === false, 'Picker aktiv, wenn tournamentId gesetzt');
  expect(
    /Hochladen|Febor/i.test(uploadResult.beforeStatus) || uploadResult.beforeStatus === '',
    `Status initial leer oder harmlos (real: „${uploadResult.beforeStatus?.slice(0, 60)}…")`
  );
  expect(uploadResult.immediateDisabled === true, 'Picker wird während Upload sofort disabled');
  expect(
    /Wird hochgeladen/i.test(uploadResult.immediateStatus),
  ...
  expect(
    /hochgeladen/i.test(afterUpload.statusText),
    `Status nach Erfolg: "Logo hochgeladen." (real: „${afterUpload.statusText?.slice(0, 60)}…")`
  );
  expect(
    afterUpload.statusClass.includes('is-ok'),
  ...
  expect(
    afterUpload.imgSrc && afterUpload.imgSrc.includes('/api/tournaments/t-42/logo'),
  ...
  expect(
    afterUpload.fetchCalls.length === 1,
    `Genau 1 Fetch-Aufruf an /logo (real: ${afterUpload.fetchCalls.length})`
  );
  expect(afterUpload.fetchCalls[0].method === 'POST', 'Fetch-Methode ist POST');
  ...
  const focusCheck = await evalPageFn(() => {
    ...
    const inputs = Array.from(
      document.querySelectorAll('.t-wizard input.t-input, .t-wizard textarea.t-input')
    );
    const nameInput = inputs[0];
    const dateInput = inputs[1];
    if (!nameInput || !dateInput)
      return { ok: false, why: 'inputs-not-found', count: inputs.length };
  ...
  expect(
    focusCheck.focusedAfterChange,
    `Fokus bleibt auf Name-Input direkt nach change-Event (active: ${focusCheck.activeTag})`
  );
  expect(focusCheck.nameIsSame, 'Name-Input ist DASSELBE DOM-Element — kein Re-Render');
  ...
  const removeClick = await evalPageFn(() => {
    ...
    return {
      ok: true,
      immediateStatus: document.querySelector('.t-wizard-logo-status').textContent,
    };
  });
  ...
  expect(
    afterRemove.status === '' || afterRemove.status === undefined,
  ...
  expect(
    /Fehler/i.test(afterError.statusText),
    `Status zeigt "Fehler: …" (real: „${afterError.statusText?.slice(0, 80)}…")`
  );
  expect(
    afterError.statusClass.includes('is-error'),
    `Status-Klasse is-error (real: ${afterError.statusClass})`
  );
  expect(afterError.pickBtnDisabled === false, 'Picker-Button wieder aktiv nach Fehler');
  expect(
    afterError.fileInputValue === '',
    'File-Input zurückgesetzt (sonst kann User dieselbe Datei nicht erneut wählen)'
  );
  expect(afterError.previewHidden === true, 'Preview weiterhin versteckt');
