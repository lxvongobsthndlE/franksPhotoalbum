...
const spiel = (o) => ({
  ...
  scheduledAt: '2026-04-18T' + (o.zeit || '14:00') + ':00.000Z',
  field: o.platte ?? 1,
  label: o.gruppe ?? 'Gruppe A',
  isGroupMatch: !o.ko,
  isKo: !!o.ko,
  stageType: o.ko ? 'ko' : 'group',
  round: o.runde,
  roundLabel: o.rlabel,
  bracketPos: o.pos,
});
...
const standings = [
  {
    ...
    groupName: 'Gruppe A',
    standings: [
      st('Blaue Hummeln', 3, 2, 0, 1, 12, 6, 6),
      st('Netzroller', 2, 1, 1, 0, 8, 6, 4),
      st('Tafelrunde', 3, 0, 2, 1, 7, 8, 2),
      st('Kellerkinder', 2, 0, 0, 2, 2, 9, 0),
    ],
  },
  {
    groupKey: 'B',
    groupName: 'Gruppe B',
    standings: [
      st('Schmetterhand', 3, 3, 0, 0, 11, 3, 9),
      st('Effet & Co.', 2, 1, 0, 1, 6, 5, 3),
      st('Halbdistanz', 3, 1, 0, 2, 5, 7, 3),
      st('Randlage', 2, 0, 0, 2, 1, 8, 0),
    ],
  },
  {
    groupKey: 'C',
    groupName: 'Gruppe C',
    standings: [
      st('Topspin Bande', 2, 2, 0, 0, 8, 3, 6),
      st('Stille Post', 2, 1, 0, 1, 5, 5, 3),
      st('Die Unbeugsamen', 1, 0, 0, 1, 2, 2, 1),
      st('Kantenglück', 1, 0, 0, 1, 1, 6, 0),
    ],
  },
];
const bestThirds = [
...
const groups = standings.map((g, i) => ({
  ...
  key: g.groupKey,
  name: g.groupName,
  members: g.standings.map((s) => ({
    teamId: s.teamId,
    name: s.name,
    color: byName[s.name]?.color,
  })),
}));
...
const tournament = {
  ...
  location: 'Vereinsheim, Halle 2',
  mode: 'groups_ko',
  isAdmin: true,
  rules:
    'Wer 15 Minuten nach Ansetzung nicht am Tisch steht, verliert das Spiel 0:3.\n\nGetränke gehen aufs Haus, wenn ihr die Becher selbst wieder einsammelt.',
  config: {
    fields: [
...
function detailSchale() {
  const tabs = [
    ...
    .map(
      ([v, l], i) =>
        `<button type="button" class="t-mod-tab${i === 0 ? ' is-active' : ''}" data-view="${v}"><span>${l}</span></button>`
    )
    .join('');
  return `
...
if (view === 'liste') {
  ...
  const gruppe = (
    titel,
    eintraege
  ) => `<section class="t-list-group" data-phase-group="${eintraege[0].phase}">
      <div class="t-list-group-label">${titel} · ${eintraege.length}</div>
      <div class="tournament-instance-grid">${eintraege
        .map((instance) =>
          renderTournamentListCard({
            instance,
  ...
  detail.querySelector('[data-tab-body="teams-mount"]').innerHTML = renderTeamsList(teams, {
    isAdmin,
    reorderable: true,
  });
  ...
  detail.querySelector('[data-tab-body="drucken-mount"]').innerHTML = renderDruckboegen(tDto, 2);
