...
const marken = (html) =>
  [
    ...html.matchAll(
      /<div class="t-zeitmarke([^"]*)">\s*<span class="t-zeitmarke-zeit">([^<]*)<\/span>[\s\S]*?<span class="t-zeitmarke-zustand">(?:<span[^>]*><\/span>)?([^<]*)<\/span>/g
    ),
...
describe('renderMatchList — Zeitachse', () => {
  ...
  it('die Reihenfolge der Bloecke folgt der Eingabe, nicht der Uhrzeit-Sortierung', () => {
    ...
    const html = renderMatchList(
      [m({ id: '1', zeit: '15:00' }), m({ id: '2', zeit: '14:00', fin: true, hs: 3, as: 1 })],
      false
    );
